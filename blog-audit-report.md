...
## Audit: Case Study Part 4 (2026-05-01)

**Audit Date:** 2026-05-01  
**Post Audited:** `how-i-built-the-local-search-ally-website-part-4-ui-ux-review-refinement.mdx`

### Summary Dashboard

| Metric                       | Count                   |
| ---------------------------- | ----------------------- |
| Quality score — before fixes | **48/100**              |
| Quality score — after fixes  | **~81/100** (estimated) |
| Critical issues found        | 6                       |
| Orphan pages                 | 0 (linked from Part 3)  |
| Stale content                | 0 (published today)     |

### Per-Post Scores

| Category        | Score  | Max     | Notes                                                                                       |
| --------------- | ------ | ------- | ------------------------------------------------------------------------------------------- |
| Content Quality | 10     | 25      | Short (~800 words), placeholder artifacts throughout                                        |
| SEO             | 11     | 20      | Title too long (72 chars), no explicit H1, no image alt text                                |
| E-E-A-T         | 15     | 20      | Good first-person voice, sourced stats, real-world data                                     |
| Technical       | 8      | 15      | `[IMAGE:]` placeholders render as raw text; internal link placeholders are not actual links |
| AI Citation     | 4      | 20      | Blockquote Takeaways is good; citation/insight markers are still template text              |
| **Total**       | **48** | **100** |                                                                                             |

### Critical Issues Found

#### 🔴 Issue 1 — IMAGE placeholders not rendered (Critical)

All 4 image references in the post body are still template text, not real `<Image />` components. They render as raw visible text on the published blog.

**Fix:** Replace each with an `<Image src="..." alt="..." />` MDX component using actual files in `/public/images/`.

#### 🔴 Issue 2 — Internal links are placeholder text (Critical)

Two internal links are still template markers, not actual clickable links:

```
[INTERNAL-LINK: check out the services page for the result → /services]
[INTERNAL-LINK: view the homepage to see the guide-first strategy → /]
```

**Fix:** Replace with proper Markdown links.

#### 🔴 Issue 3 — Citation/Insight markers are raw template text (Critical)

Four content markers still in template format — render as literal text on page:
...
**Fix:** Strip marker prefix; incorporate as clean prose.

#### 🔴 Issue 4 — Duplicate "Meta Description" section at the bottom (High)

Leftover template artifact — description already in frontmatter. **Fix:** Remove the section.

#### 🟡 Issue 5 — Title too long for SEO (Medium)

Title is 72 characters — over the 50-60 char ideal. **Fix:** Shorten.

#### 🟡 Issue 6 — Missing `author` frontmatter field (Medium)

Required for JSON-LD schema. **Fix:** Add `author: "Chad Smith"`.

### Prioritized Action Queue

| Priority | Issue                              | Action                              |
| -------- | ---------------------------------- | ----------------------------------- |
| 1        | IMAGE placeholders render as text  | Replace with `<Image />` components |
| 2        | INTERNAL-LINK placeholders         | Convert to real Markdown links      |
| 3        | Citation marker artifacts          | Strip markers, clean prose          |
| 4        | Duplicate Meta Description section | Remove the section                  |
| 5        | Missing `author` field             | Add to frontmatter                  |
| 6        | Title length (72 chars)            | Shorten the title                   |

---

## Previous Audit: local-seo-for-plumbers.mdx (2026-04-26)

**Audit Date:** 2026-04-26  
**Total Posts at time:** 15  
...
| Topic                      | Posts                                                                                   | Verdict                                                                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Google reviews             | `how-to-get-more-google-reviews-as-a-contractor` + `contractor-google-reviews-nwa`      | **Differentiate** — first post covers the system (process, cadence); second covers review _content quality_ for AI/ranking. Different intents, but titles are similar. Clarify in each title/meta which angle it covers. |
